...
import { ApiVersion, AuthScopes, BillingInterval, DeliveryMethod, FeatureDeprecatedError, HttpResponseError, InvalidJwtError, RequestedTokenType, ShopifyError, ShopifyHeader, WebhookType, WebhookValidationErrorReason, shopifyApi } from "@shopify/shopify-api";
import { jsx } from "react/jsx-runtime";
import { compare } from "compare-versions";
import { PrismaSessionStorage } from "@shopify/shopify-app-session-storage-prisma";
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/const.mjs
...
var RETRY_INVALID_SESSION_HEADER = { "X-Shopify-Retry-Invalid-Session-Request": "1" };
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/helpers/app-bridge-url.mjs
var appBridgeUrlOverride;
function setAppBridgeUrlOverride(url) {
	appBridgeUrlOverride = url;
}
function appBridgeUrl() {
	return appBridgeUrlOverride || "https://cdn.shopify.com/shopifycloud/app-bridge.js";
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/types.mjs
var AppDistribution;
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/adapters/node/index.mjs
setAbstractRuntimeString(() => {
	return `React Router (Node)`;
});
if (process.env.APP_BRIDGE_URL) setAppBridgeUrlOverride(process.env.APP_BRIDGE_URL);
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/boundary/headers.mjs
function headersBoundary(headers) {
	const { parentHeaders, loaderHeaders, actionHeaders, errorHeaders } = headers;
	if (errorHeaders && Array.from(errorHeaders.entries()).length > 0) return errorHeaders;
	return new Headers([
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/boundary/error.mjs
function errorBoundary(error) {
	if (error.constructor.name === "ErrorResponse" || error.constructor.name === "ErrorResponseImpl") return jsx("div", { dangerouslySetInnerHTML: { __html: error.data || "Handling response" } });
	throw error;
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/boundary/index.mjs
/**
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/version.mjs
var SHOPIFY_REACT_ROUTER_LIBRARY_VERSION = "1.2.0";
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/webhooks/register.mjs
		...
		return api.webhooks.register({ session }).then((response) => {
			Object.entries(response).forEach(([topic, topicResults]) => {
				topicResults.forEach(({ success, ...rest }) => {
					if (success) logger.debug("Registered webhook", {
						topic,
						shop: session.shop,
						operation: rest.operation
					});
					else logger.error("Failed to register webhook", {
						topic,
						shop: session.shop,
			...
			if ((error.body?.errors?.graphQLErrors || []).find(({ extensions: { code } }) => code === "THROTTLED")) logger.error("Failed to register webhooks", {
				shop: session.shop,
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/helpers/ensure-cors-headers.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/redirect-to-bounce-page.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/helpers/respond-to-invalid-session-token.mjs
function respondToInvalidSessionToken({ params, request, retryRequest = false }) {
	const { api, logger, config } = params;
	if (!request.headers.get("authorization")) return redirectToBouncePage({ config }, new URL(request.url));
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/helpers/get-shop-from-request.mjs
function getShopFromRequest(request) {
	return new URL(request.url).searchParams.get("shop");
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/helpers/validate-session-token.mjs
async function validateSessionToken(params, request, token, { checkAudience = true, retryRequest = true } = {}) {
	...
	try {
		const payload = await api.session.decodeSessionToken(token, { checkAudience });
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/helpers/get-session-token-header.mjs
var SESSION_TOKEN_PARAM$1 = "id_token";
function getSessionTokenHeader(request) {
	return request.headers.get("authorization")?.replace("Bearer ", "");
}
function getSessionTokenFromUrlParam(request) {
	return new URL(request.url).searchParams.get(SESSION_TOKEN_PARAM$1);
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/helpers/reject-bot-request.mjs
var SHOPIFY_USER_AGENTS = [/Shopify POS\//, /Shopify Mobile\//];
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/helpers/respond-to-options-request.mjs
function respondToOptionsRequest(params, request, corsHeaders) {
	if (request.method === "OPTIONS") throw ensureCORSHeadersFactory(params, request, corsHeaders)(new Response(null, {
		status: 204,
		headers: { "Access-Control-Max-Age": "7200" }
	}));
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/helpers/invalidate-access-token.mjs
async function invalidateAccessToken(params, session) {
	const { logger, config } = params;
	logger.debug(`Invalidating access token for session - ${session.id}`, { shop: session.shop });
	session.accessToken = void 0;
	await config.sessionStorage.storeSession(session);
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/billing/cancel.mjs
	...
	return async function cancelBilling(options) {
		...
		try {
			...
			if (error instanceof HttpResponseError && error.response.code === 401) {
				logger.debug("API token was invalid, responding to invalid session", { shop: session.shop });
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/billing/require.mjs
function requireBillingFactory(params, request, session) {
	...
	return async function requireBilling(options) {
		...
		if (!data.hasActivePayment) {
			...
			throw await options.onFailure(/* @__PURE__ */ new Error("Billing check failed"));
		}
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/redirect-with-app-bridge-headers.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/billing/helpers.mjs
function redirectOutOfApp(params, request, url, shop) {
	...
	if (request.headers.get("authorization")) throw new Response(void 0, {
		status: 401,
		statusText: "Unauthorized",
	...
	} else throw redirect(url);
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/billing/request.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/billing/check.mjs
function checkBillingFactory(params, request, session) {
	return async function checkBilling(options = {}) {
		const { api, logger } = params;
		logger.debug("Checking billing plans", {
			shop: session.shop,
			...options
		});
		try {
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/billing/create-usage-record.mjs
function createUsageRecordFactory(params, request, session) {
	return async function createUsageRecord(options) {
		const { api, logger } = params;
		logger.debug("Create usage record", {
			shop: session.shop,
			...options
		});
		try {
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/billing/update-usage-subscription-capped-amount.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/clients/admin/graphql.mjs
function graphqlClientFactory({ params, handleClientError, session }) {
	return async function query(operation, options) {
		...
		try {
			...
			if (handleClientError) throw await handleClientError({
				error,
				params,
				session
			});
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/clients/admin/factory.mjs
function adminClientFactory({ params, handleClientError, session }) {
	return { graphql: graphqlClientFactory({
		params,
		session,
		handleClientError
	}) };
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/create-admin-api-context.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/redirect-to-shopify-or-app-root.mjs
async function redirectToShopifyOrAppRoot(request, params, responseHeaders) {
	...
	let redirectUrl;
	if (config.distribution === AppDistribution.ShopifyAdmin) redirectUrl = `/?shop=${shop}&host=${encodeURIComponent(host)}`;
	else redirectUrl = await api.auth.getEmbeddedAppUrl({ rawRequest: request });
	throw redirect(redirectUrl, { headers: responseHeaders });
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/ensure-app-is-embedded-if-required.mjs
var ensureAppIsEmbeddedIfRequired = async (params, request) => {
	...
	if (config.distribution !== AppDistribution.ShopifyAdmin && url.searchParams.get("embedded") !== "1") {
		...
		await redirectToShopifyOrAppRoot(request, {
			api,
			config
		});
	}
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/ensure-session-token-search-param-if-required.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/helpers/add-response-headers.mjs
function addDocumentResponseHeadersFactory(params) {
	...
	return function(request, headers) {
		...
		addDocumentResponseHeaders$1(headers, config.distribution !== AppDistribution.ShopifyAdmin, shop);
	};
}
function addDocumentResponseHeaders$1(headers, isEmbeddedApp, shop) {
	if (shop) headers.set("Link", `<${CDN_URL}>; rel="preconnect", <${APP_BRIDGE_URL}>; rel="preload"; as="script", <${POLARIS_URL}>; rel="preload"; as="script"`);
	if (isEmbeddedApp) {
		if (shop) headers.set("Content-Security-Policy", `frame-ancestors https://${shop} https://admin.shopify.com https://*.spin.dev https://admin.myshopify.io https://admin.shop.dev;`);
	} else headers.set("Content-Security-Policy", `frame-ancestors 'none';`);
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/validate-redirect-url.mjs
var FILE_URI_MATCH = /\/\/\//;
...
function isSafe(domain, redirectUrl, requireSSL = true) {
	...
	if (FILE_URI_MATCH.test(redirectUrl) || WHITESPACE_CHARACTER.test(redirectUrl)) return false;
	let url;
	try {
	...
	if (INVALID_RELATIVE_URL.test(url.pathname)) return false;
...
function sanitizeRedirectUrl(domain, redirectUrl, options = {}) {
	if (isSafe(domain, redirectUrl, options.requireSSL)) return new URL(redirectUrl, domain);
	else if (options.throwOnInvalid === false) return;
	else throw new ShopifyError("Invalid URL. Refusing to redirect");
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/render-app-bridge.mjs
function renderAppBridge({ config }, request, redirectTo) {
	...
	addDocumentResponseHeaders$1(responseHeaders, config.distribution !== AppDistribution.ShopifyAdmin, new URL(request.url).searchParams.get("shop"));
	throw new Response(`
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/redirect.mjs
function redirectFactory(params, request, shop) {
	...
	return function redirect$1(url, init) {
		...
		if (parsedUrl.origin === config.appUrl || url.startsWith("/")) searchParams.forEach((value, key) => {
			if (!parsedUrl.searchParams.has(key)) parsedUrl.searchParams.set(key, value);
		});
		if (target === "_self") if (isBounceRequest(request)) throw renderAppBridge(params, request, {
			url: parsedUrl.toString(),
			target
		});
		else return redirect(parsedUrl.toString(), init);
		else if (isDataRequest(request)) throw redirectWithAppBridgeHeaders(parsedUrl.toString());
		else if (isEmbeddedRequest(request)) throw renderAppBridge(params, request, {
			url: parsedUrl.toString(),
			target
		});
...
function parseURL({ params, base, init, shop, url }) {
	...
	if (isAdminRemotePath(url)) {
		...
		if (!target) target = config.distribution === AppDistribution.ShopifyAdmin ? "_self" : "_parent";
		return {
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/validate-shop-and-host-params.mjs
function validateShopAndHostParams(params, request) {
	...
	if (config.distribution !== AppDistribution.ShopifyAdmin) {
		...
		const shop = api.utils.sanitizeShop(url.searchParams.get("shop"));
		if (!shop) {
			logger.debug("Missing or invalid shop, redirecting to login path", { shop });
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/redirect-to-install-page.mjs
async function redirectToInstallPage(params, shop, optionalScopes = []) {
	...
	if (params.config.distribution === AppDistribution.ShopifyAdmin) throw redirect(installUrl);
	else throw redirectWithAppBridgeHeaders(installUrl);
}
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/scope/client/fetch-scopes-details.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/scope/request.mjs
function requestScopesFactory(params, session, admin) {
	...
	async function alreadyGranted(scopes, admin) {
		return new AuthScopes((await fetchScopeDetail(admin)).app.installation.accessScopes.map((scope) => scope.handle)).has(scopes);
	}
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/scope/query.mjs
function queryScopesFactory(params, session, admin) {
	return async function queryScopes() {
		const { logger } = params;
		logger.debug("Querying scopes details: ", { shop: session.shop });
		return mapFetchScopeDetail(await fetchScopeDetail(admin));
	};
}
function mapFetchScopeDetail(scopesDetailResponse) {
	const appInformation = scopesDetailResponse.app;
	return {
		granted: new AuthScopes(appInformation.installation.accessScopes.map((scope) => scope.handle)).toArray(true),
		required: new AuthScopes(appInformation.requestedAccessScopes.map((scope) => scope.handle)).toArray(true),
		optional: new AuthScopes(appInformation.optionalAccessScopes.map((scope) => scope.handle)).toArray(true)
	};
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/scope/client/revoke-scopes.mjs
...
async function revokeScopes(admin, scopes) {
	return (await (await admin.graphql(REVOKE_SCOPE_MUTATION, { variables: { scopes } })).json()).data.appRevokeAccessScopes;
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/scope/revoke.mjs
function revokeScopesFactory(params, session, admin) {
	return async function revoke(scopes) {
		...
		return { revoked: revokeScopesResult.revoked.map((scope) => scope.handle) };
	};
}
async function validateScopes(scopes) {
	if (!scopes || scopes.length === 0) throw new Response("No scopes provided", { status: 400 });
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/scope/factory.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/authenticate.mjs
function authStrategyFactory({ strategy, ...params }) {
	const { api, logger, config } = params;
	async function respondToBouncePageRequest(request) {
		if (new URL(request.url).pathname.endsWith(config.auth.patchSessionTokenPath)) {
			logger.debug("Rendering bounce page", { shop: getShopFromRequest(request) });
	...
	function addEmbeddedFeatures(context, request, session, sessionToken) {
		if (config.distribution === AppDistribution.ShopifyAdmin) return context;
		return {
			...context,
			sessionToken,
	...
	return async function authenticateAdmin(request) {
		try {
			...
			logger.info("Authenticating admin request", { shop: getShopFromRequest(request) });
			...
			return createContext(request, await strategy.authenticate(request, {
			...
			if (errorOrResponse instanceof Response) {
				logger.debug("Authenticate returned a response", { shop: getShopFromRequest(request) });
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/handle-client-error.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/helpers/create-or-load-offline-session.mjs
async function createOrLoadOfflineSession({ api, config, logger }, shop) {
	if (config.distribution === AppDistribution.ShopifyAdmin) {
		logger.debug("Creating custom app session from configured access token", { shop });
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/helpers/refresh-token.mjs
async function refreshToken(params, shop, refreshToken) {
	...
	try {
		...
		if (error instanceof InvalidJwtError || error instanceof HttpResponseError && error.response.code === 400 && error.response.body?.error === "invalid_subject_token") throw error;
		throw new Response(void 0, {
			status: 500,
...
async function ensureOfflineTokenIsNotExpired(session, params, shop) {
	const { config } = params;
	if (config.future?.expiringOfflineAccessTokens && session.isExpired(3e5) && config.distribution !== AppDistribution.ShopifyAdmin && session.refreshToken) {
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/helpers/ensure-valid-offline-session.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/webhooks/authenticate.mjs
function authenticateWebhookFactory(params) {
	...
	return async function authenticate(request) {
		...
		if (!check.valid) if (check.reason === WebhookValidationErrorReason.InvalidHmac) {
		...
		let webhookContext;
		if (check.webhookType === WebhookType.Webhooks) webhookContext = {
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/override-logger.mjs
function overrideLogger(logger) {
	...
	function deprecated(warningFunction) {
		return function(version, message) {
			if (compare("1.2.0", version, ">=")) throw new FeatureDeprecatedError(`Feature was deprecated in version ${version}`);
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/login/login.mjs
function loginFactory(params) {
	const { api, config, logger } = params;
	return async function login(request) {
		const shopParam = new URL(request.url).searchParams.get("shop");
		if (request.method === "GET" && !shopParam) return {};
		...
		const installPath = `https://${api.utils.legacyUrlToShopAdminUrl(sanitizedShop)}/oauth/install?client_id=${config.apiKey}`;
		const redirectUrl = config.distribution !== AppDistribution.ShopifyAdmin ? installPath : authPath;
		logger.info(`Redirecting login request to ${redirectUrl}`, { shop: sanitizedShop });
		throw redirect(redirectUrl);
	};
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/errors.mjs
var SessionNotFoundError = class extends ShopifyError {};
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/unauthenticated/admin/factory.mjs
function unauthenticatedAdminContextFactory(params) {
	return async (shop) => {
		const session = await ensureValidOfflineSession(params, shop);
		if (!session) throw new SessionNotFoundError(`Could not find a session for shop ${shop} when creating unauthenticated admin context`);
		return {
			session,
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/public/extension/authenticate.mjs
function authenticateExtensionFactory(params, requestType) {
	return async function authenticateExtension(request, options = {}) {
		...
		logger.info(`Authenticating ${requestType} request`, { shop: getShopFromRequest(request) });
		if (!sessionTokenHeader) {
			logger.debug("Request did not contain a session token", { shop: getShopFromRequest(request) });
			throw new Response(void 0, {
				status: 401,
				statusText: "Unauthorized"
			});
		...
		return {
			sessionToken: await validateSessionToken(params, request, sessionTokenHeader, {
				checkAudience: false,
				retryRequest: false
			}),
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/public/checkout/authenticate.mjs
function authenticateCheckoutFactory(params) {
	return authenticateExtensionFactory(params, "checkout");
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/clients/storefront/factory.mjs
function storefrontClientFactory({ params, session }) {
	const { api } = params;
	return { graphql: async (query, options = {}) => {
		const apiResponse = await new api.clients.Storefront({
			session,
			apiVersion: options.apiVersion
		}).request(query, {
			variables: options?.variables,
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/public/appProxy/authenticate.mjs
...
var liquid = (body, initAndOptions) => {
	...
	if (typeof initAndOptions !== "object") return new Response(processedBody, {
...
async function validateAppProxyHmac(params, url) {
	...
	try {
		...
		if (!isValid) {
			const data = `routes%2F${url.pathname.replace(/^\//, "").replace(/\/$/, "").replaceAll("/", ".")}`;
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/public/customer-account/authenticate.mjs
function authenticateCustomerAccountFactory(params) {
	return authenticateExtensionFactory(params, "customer account");
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/public/factory.mjs
function authenticatePublicFactory(params) {
	return {
		checkout: authenticateCheckoutFactory(params),
		appProxy: authenticateAppProxyFactory(params),
		customerAccount: authenticateCustomerAccountFactory(params)
	};
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/unauthenticated/storefront/factory.mjs
function unauthenticatedStorefrontContextFactory(params) {
	return async (shop) => {
		const session = await ensureValidOfflineSession(params, shop);
		if (!session) throw new SessionNotFoundError(`Could not find a session for shop ${shop} when creating unauthenticated storefront context`);
		return {
			session,
			storefront: storefrontClientFactory({
				params,
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/helpers/trigger-after-auth-hook.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/strategies/token-exchange.mjs
var createTokenExchangeStrategy = (params) => {
	...
	async function exchangeToken({ request, shop, sessionToken, requestedTokenType }) {
		try {
			...
			if (error instanceof InvalidJwtError || error instanceof HttpResponseError && error.response.code === 400 && error.response.body?.error === "invalid_subject_token") throw respondToInvalidSessionToken({
				params: {
					api,
					config,
					logger
	...
	async function authenticate(request, sessionContext) {
		...
		if (!session || !session.isActive(void 0, 3e5)) {
			...
			try {
				...
				if (errorOrResponse instanceof Response) throw errorOrResponse;
				throw new Response(void 0, {
					status: 500,
					statusText: "Internal Server Error"
				});
		...
		return handleClientErrorFactory({
			...
			onError: async ({ session, error }) => {
				if (error.response.code === 401) {
					logger.debug("Responding to invalid access token", { shop: getShopFromRequest(request) });
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/admin/strategies/merchant-custom-app.mjs
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/helpers/idempotent-promise-handler.mjs
...
var IdempotentPromiseHandler = class {
	...
	async handlePromise({ promiseFunction, identifier }) {
		try {
			if (this.isPromiseRunnable(identifier)) await promiseFunction();
		} finally {
	...
	async clearStaleIdentifiers() {
		this.identifiers.forEach((date, identifier, map) => {
			if (Date.now() - date > IDENTIFIER_TTL_MS) map.delete(identifier);
		});
	}
};
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/flow/authenticate.mjs
function authenticateFlowFactory(params) {
	const { api, logger } = params;
	return async function authenticate(request) {
		logger.info("Authenticating flow request");
		if (request.method !== "POST") {
			logger.debug("Received a non-POST request for flow. Only POST requests are allowed.", {
				url: request.url,
				method: request.method
			});
			throw new Response(void 0, {
		...
		logger.debug("Flow request is valid, looking for an offline session", { shop: payload.shopify_domain });
		const session = await ensureValidOfflineSession(params, payload.shopify_domain);
		if (!session) {
			logger.info("Flow request could not find session", { shop: payload.shopify_domain });
			throw new Response(void 0, {
				status: 400,
				statusText: "Bad Request"
			});
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/fulfillment-service/authenticate.mjs
function authenticateFulfillmentServiceFactory(params) {
	const { api, logger } = params;
	return async function authenticate(request) {
		logger.info("Authenticating fulfillment service request");
		if (request.method !== "POST") {
			logger.debug("Received a non-POST request for fulfillment service. Only POST requests are allowed.", {
				url: request.url,
				method: request.method
			});
			throw new Response(void 0, {
		...
		if (!result.valid) {
			logger.error("Received an invalid fulfillment service request", { reason: result.reason });
			throw new Response(void 0, {
				status: 400,
				statusText: "Bad Request"
			});
		...
		logger.debug("Fulfillment service request is valid, looking for an offline session", { shop });
		const session = await ensureValidOfflineSession(params, shop);
		if (!session) {
			logger.info("Fulfillment service request could not find session", { shop });
			throw new Response(void 0, {
				status: 400,
				statusText: "Bad Request"
			});
		}
		logger.debug("Found a session for the fulfillment service request", { shop });
		return {
			session,
			payload,
			admin: adminClientFactory({
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/authenticate/pos/authenticate.mjs
function authenticatePOSFactory(params) {
	return authenticateExtensionFactory(params, "pos");
}
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/shopify-app.mjs
/**
...
function shopifyApp(appConfig) {
	...
	if (appConfig.webhooks) api.webhooks.addHandlers(appConfig.webhooks);
	const params = {
		api,
		config,
		logger
	...
	let strategy;
	if (config.distribution === AppDistribution.ShopifyAdmin) strategy = createMerchantCustomAuthStrategy(params);
	else strategy = createTokenExchangeStrategy(params);
	...
	if (isAppStoreApp(shopify, appConfig) || isSingleMerchantApp(shopify, appConfig)) shopify.login = loginFactory(params);
	return shopify;
}
...
function deriveConfig(appConfig, apiConfig) {
	if (!appConfig.sessionStorage && appConfig.distribution !== AppDistribution.ShopifyAdmin) throw new ShopifyError("Please provide a valid session storage. Refer to https://github.com/Shopify/shopify-app-js/blob/main/README.md#session-storage-options for options.");
...
//#endregion
//#region node_modules/@shopify/shopify-app-react-router/dist/esm/server/index.mjs
setAbstractRuntimeString(() => {
	return `React Router`;
});
//#endregion
//#region app/shopify.server.js
var MONTHLY_PLAN = "pro_monthly";
var ANNUAL_PLAN = "pro_annual";
var scopes = (process.env.SCOPES || "").split(",").map((s) => s.trim()).filter(Boolean);
var shopify = shopifyApp({
	...
	billing: {
		[MONTHLY_PLAN]: { lineItems: [{
			amount: 19.9,
			currencyCode: "USD",
			interval: BillingInterval.Every30Days
		}] },
		[ANNUAL_PLAN]: { lineItems: [{
			amount: 200,
			currencyCode: "USD",
...
//#endregion
export { login as a, boundary as c, authenticate as i, LoginErrorType as l, MONTHLY_PLAN as n, registerWebhooks as o, addDocumentResponseHeaders as r, shopify as s, ANNUAL_PLAN as t };
