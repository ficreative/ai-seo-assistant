import { t as require_index_browser } from "./@prisma_client.js";
import { i as Session } from "./lib-DegJ33eS.js";
//#region node_modules/@shopify/shopify-app-session-storage-prisma/dist/esm/prisma.mjs
...
var PrismaSessionStorage = class {
	...
	constructor(prisma, { tableName, connectionRetries, connectionRetryIntervalMs } = {}) {
		...
		if (connectionRetries !== void 0) this.connectionRetries = connectionRetries;
		if (connectionRetryIntervalMs !== void 0) this.connectionRetryIntervalMs = connectionRetryIntervalMs;
		if (this.getSessionTable() === void 0) throw new Error(`PrismaClient does not have a ${this.tableName} table`);
	...
	async storeSession(session) {
		...
		try {
			...
			if (error instanceof import_index_browser.Prisma.PrismaClientKnownRequestError && error.code === UNIQUE_KEY_CONSTRAINT_ERROR_CODE) {
	...
	async loadSession(id) {
		await this.ensureReady();
		const row = await this.getSessionTable().findUnique({ where: { id } });
		if (!row) return;
	...
	async findSessionsByShop(shop) {
		await this.ensureReady();
		return (await this.getSessionTable().findMany({
			where: { shop },
			take: 25,
	...
	rowToSession(row) {
		...
		if (row.refreshTokenExpires) sessionParams.refreshTokenExpires = row.refreshTokenExpires.getTime();
